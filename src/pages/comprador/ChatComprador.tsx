...
const RESPUESTAS_AUTO = [
  'Perfecto, anotado. ¿Necesitás factura A o B?',
  'Sí, sin problema. Te confirmo los detalles por este mismo canal.',
  'Entendido. ¿Tenés alguna preferencia de horario para la entrega?',
  'Mañana a primera hora sale el envío. Te mando el número de seguimiento.',
];
...
export default function ChatComprador() {
  ...
  const ordenActiva =
    ordenes.find((o) => o.compradorId === COMPRADOR_ID && o.chatHabilitado) ?? null;
  ...
  const enviarMensaje = () => {
    ...
    setTimeout(() => {
      ...
      useChatStore.getState().agregarMensaje({
        ...
        ordenId: ordenActiva.id,
        autorRol: 'proveedor',
        autorNombre: ordenActiva.proveedorNombre,
        texto: respuesta,
        timestamp: new Date().toISOString(),
      });
    ...
    return (
      <div>
        ...
        <EmptyState
          ...
          mensaje="Aceptá una cotización para habilitar la comunicación directa con el proveedor"
          accion={{
    ...
    <div>
      ...
      <Card padding="none" className="flex flex-col h-[calc(100vh-12rem)]">
        <div className="px-5 py-4 border-b border-ep-border bg-ep-surface flex items-center gap-3 flex-shrink-0">
          ...
          <span className="font-semibold text-ep-text-primary text-sm">
            {ordenActiva.proveedorNombre}
          </span>
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-3">
          {mensajesOrden.map((msg) => {
            const esComprador = msg.autorRol === 'comprador';
            return (
              <div key={msg.id} className={`flex ${esComprador ? 'justify-end' : 'justify-start'}`}>
                <div className="max-w-[75%]">
                  ...
                  <p className={`text-xs text-ep-text-muted mt-1 ${esComprador ? 'text-right' : ''}`}>
          ...
          {escribiendo && (
            <div className="flex justify-start">
              <div className="max-w-[75%]">
                <p className="text-xs font-medium text-ep-text-muted mb-1">
                  {ordenActiva.proveedorNombre}
                </p>
